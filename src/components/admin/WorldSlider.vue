...
export default {
  ...
  async created() {
    try {
      this.worlds = await topics.get().then((querySnapshot) => {
        let auxworlds = [];
  ...
  methods: {
    ...
    close() {
      this.dialog = false;
      this.$nextTick(() => {
        this.editedworld = Object.assign({}, this.defaultworld);
        this.editedIndex = -1;
      });
    },
    async save() {
      if (this.editedIndex > -1) {
        await topics
          ...
          .catch((error) => {
            ...
            this.$store.commit(
              "ACTIVE_SNACK",
              "Se produjo un error al actualizar el mundo, Intente nuevamente"
            );
          });
      } else {
        await topics
          .add({ name: this.editedworld.name })
          .then((data) => {
            this.editedworld.docId = data.id;
            this.worlds.push(this.editedworld);
            this.$store.commit(
              "ACTIVE_SNACK",
              "El mundo se registro correctamente"
            );
          })
          .catch(() => {
            this.$store.commit(
              "ACTIVE_SNACK",
              "Se produjo un error al cargar el mundo, Intente nuevamente"
            );
          });
      }
      this.close();
    ...
    async deleteItem(item) {
      ...
      var result = confirm(
        `Seguro que quiere Eliminar el mundo ${item.name} ?`
      );
      if (result == true) {
        await topics
          .doc(item.docId)
          ...
          .then(() => {
            ...
            this.$store.commit(
              "ACTIVE_SNACK",
              "El mundo se elimino correctamente"
            );
          })
          .catch(() => {
            this.$store.commit(
              "ACTIVE_SNACK",
              "Se produjo un error al eliminar el mundo, Intente nuevamente"
            );
          });
      }
    },
