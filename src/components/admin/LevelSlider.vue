  ...
  <v-container class="d-flex flex-column justify-center align-center back-con">
    <v-toolbar style="width: 100%" flat class="tenloFondo">
      <v-toolbar-title class="text--primary font-weight-large"
        >Niveles</v-toolbar-title
      >
      <v-divider class="mx-4" inset vertical></v-divider>
      <v-spacer></v-spacer>
      <v-dialog dark v-model="dialog" max-width="500px">
        <template v-slot:activator="{ on }">
          <v-btn :disabled="!world" color="orange" class="mb-2" v-on="on"
    ...
    <Scroller :elementHeigth="150" :elementsSize="levels.length">
      <p v-show="!world">No hay mundo seleccionado</p>
      <v-card
        class="my-5 py-5"
...
export default {
  ...
  methods: {
    ...
    close() {
      this.dialog = false;
      this.$nextTick(() => {
        this.editedLevel = Object.assign({}, this.defaultLevel);
        this.editedIndex = -1;
      });
    },
    async save() {
      if (this.editedIndex > -1) {
        await topics
          ...
          .then(() => {
            ...
            this.$store.commit(
              "ACTIVE_SNACK",
              "El nivel se actualizo correctamente"
            );
          })
          .catch((error) => {
            console.log(error);
            this.$store.commit(
              "ACTIVE_SNACK",
              "Se produjo un error al actualizar el nivel, Intente nuevamente"
            );
          });
      } else {
        await topics
          ...
          .then((data) => {
            this.editedLevel.docId = data.id;
          ...
          .catch(() => {
            this.$store.commit(
              "ACTIVE_SNACK",
              "Se produjo un error al cargar el nivel, Intente nuevamente"
            );
          });
      }
      this.close();
    ...
    async deleteItem(item) {
      ...
      var result = confirm(
        `Seguro que quiere Eliminar el nivel ${item.level} del mundo ${this.world.name} ?`
      );
      if (result == true) {
        await topics
          .doc(this.world.docId)
          ...
          .then(() => {
            ...
            this.$store.commit(
              "ACTIVE_SNACK",
              "El nivel se elimino correctamente"
            );
          })
          .catch(() => {
            this.$store.commit(
              "ACTIVE_SNACK",
              "Se produjo un error al eliminar el nivel, Intente nuevamente"
            );
          });
      }
    },
